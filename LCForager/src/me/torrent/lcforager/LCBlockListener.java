...
if(m != Material.GRASS && m!=Material.DIRT && m!=Material.SAND && m!=Material.GRAVEL) return;
		...
		if ((randomSaddle > 0) && (randomSaddle < 10)
				&& (level >= this.plugin.LCConfiguration.ForageSaddleLevel)) {
			...
			LCChat.info(player, "You found a lost saddle! Where's that pig?");
			LevelFunctions.addExp(player, this.plugin.thisPlug,
					this.plugin.LCConfiguration.ExpPerForageSaddle);
			return;
